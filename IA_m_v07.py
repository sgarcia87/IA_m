...
def color_node(nodo, G):
    ...
    if tipo == "equilibrio":
        ...
        return "#FF6666"  # rojo claro (polo negativo)
    elif nodo in dualidades_base.values():
        return "#66FF99"  # verde claro (polo positivo)
    else:
...
<div style="position: fixed; top: 20px; right: 20px; background-color: white; padding: 10px; border: 1px solid #ccc; font-family: sans-serif; font-size: 14px; z-index: 9999;">
  ...
  <div style="margin-top: 5px;">
    <span style="background-color: #FF6666; display: inline-block; width: 12px; height: 12px; margin-right: 5px;"></span> 🔴 Polo negativo (dualidad)<br>
    <span style="background-color: #66FF99; display: inline-block; width: 12px; height: 12px; margin-right: 5px;"></span> 🟢 Polo positivo (dualidad)<br>
...
def es_variacion_morfologica(termino1, termino2):
    """ Detecta si dos términos son variantes morfológicas, evitando falsos positivos por prefijo. """
    t1 = termino1.replace("_", " ").strip().lower()
    t2 = termino2.replace("_", " ").strip().lower()

    if t1 == t2:
        return True

    # Detectar si comparten un prefijo largo (>4) pero luego son distintos
    min_len = 4
    prefijo_comun = os.path.commonprefix([t1, t2])
    if len(prefijo_comun) >= min_len and t1 != t2:
        # Evita considerar iguales si el resto es diferente
        resto1 = t1[len(prefijo_comun):].strip()
        resto2 = t2[len(prefijo_comun):].strip()
        if resto1 and resto2 and resto1 != resto2:
            return False

    # Aplicar stemming normal
