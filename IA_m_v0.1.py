...
def color_edge(u, v, G):
    ...
    # 🟡 Nodo equilibrio
    if G.nodes.get(u, {}).get("tipo") == "equilibrio" or G.nodes.get(v, {}).get("tipo") == "equilibrio":
        color = "yellow"  
    # 🟠 Nodo síntesis
    if G.nodes.get(u, {}).get("tipo") == "sintesis" or G.nodes.get(v, {}).get("tipo") == "sintesis":
        color = "orange"
    if G.nodes.get(u, {}).get("es_dualidad") and G.nodes.get(v, {}).get("es_dualidad"):
        color = "red"
    if G.nodes.get(v, {}).get("es_dualidad") and G.nodes.get(v, {}).get("es_dualidad"):
        color = "green"
    elif v in dualidades_base and dualidades_base.get(v) == u:
        color = "green"
#    # 🟡 Equilibrio o síntesis superior
#    if "equilibrio" in u.lower() or "equilibrio" in v.lower():
#        color = "yellow"
#    if "concepto_superior" in G.nodes.get(u, {}) or "concepto_superior" in G.nodes.get(v, {}):
#        color = "yellow"
    return color
...
def generar_subgrafo_html(G, tema, top_n=100):
    ...
    <div style="position: fixed; top: 20px; right: 20px; background-color: white; padding: 10px; border: 1px solid #ccc; font-family: sans-serif; font-size: 14px; z-index: 9999;">
      ...
      <div style="margin-top: 5px;">
        ...
        <span style="background-color: #ccffcc; display: inline-block; width: 12px; height: 12px; margin-right: 5px;"></span> Nivel 3: Emergente<br>
        <span style="background-color: #ff99cc; display: inline-block; width: 12px; height: 12px; margin-right: 5px;"></span> Nivel 4: Abstracto superior
      </div>
    </div>
    """
...
def generar_indice_subgrafos(G, top_n=50, carpeta="subgrafos"):
    """Genera un HTML interactivo para acceder a los subgrafos clave"""
    ...
    for nodo in nodos_destacados:
        ...
        if os.path.exists(ruta):  # Solo enlazamos los que ya existen
            enlaces.append(f"<li><a href='{nombre_archivo}' target='_blank'>{nodo}</a></li>")

    html = f"""
    <html>
    <head>
        <meta charset="UTF-8">
        <title>🌐 Subgrafos de IA_m</title>
        ...
        <style>
            ...
            .leyenda {{
                ...
                width: 250px;
                font-size: 14px;
                z-index: 10; /* Asegura que la leyenda esté encima del resto de los elementos */
            }}
        </style>    
    </head>
    <body>
        <h1>🌐 Subgrafos principales de IA_m</h1>
        
        <input type="text" id="buscador" placeholder="Buscar nodo..." onkeyup="filtrar()">

        <!-- Leyenda visual de colores por nivel -->
        <!-- Leyenda visual de colores por nivel -->
        <div class="leyenda">
            <strong>Leyenda de niveles conceptuales:</strong><br>
            <div style="margin-top: 5px;">
                <span style="background-color: #cccccc; display: inline-block; width: 12px; height: 12px; margin-right: 5px;"></span> Nivel 0: Concepto base<br>
                <span style="background-color: #ffcc99; display: inline-block; width: 12px; height: 12px; margin-right: 5px;"></span> Nivel 1: Dualidad<br>
                <span style="background-color: #99ccff; display: inline-block; width: 12px; height: 12px; margin-right: 5px;"></span> Nivel 2: Equilibrio / Síntesis<br>
        ...
        function filtrar() {{
            ...
            var ul = document.getElementById("lista_nodos");
            var li = ul.getElementsByTagName("li");
            for (var i = 0; i < li.length; i++) {{
                var a = li[i].getElementsByTagName("a")[0];
                var txt = a.textContent || a.innerText;
                li[i].style.display = txt.toLowerCase().indexOf(filtro) > -1 ? "" : "none";
            }}
        }}
        </script>
    </body>
    ...
    with open(os.path.join(carpeta, "index.html"), "w", encoding="utf-8") as f:
        f.write(html)
    print("📂 Índice interactivo de subgrafos generado como 'subgrafos/index.html'")
    ...
    try:
        ...
        respuesta = client.chat.completions.create(
            ...
            messages=[
                ...
                "Los términos deben describir conceptualmente el tema que se proponga. Busco el concepto cualitativa o cuantitativa del término."
                "No incluyas números para ordenar, ni textos introductorios o de cierre. "
                "Si no entiendes el término, repite literalmente la palabra que se te ha dado."
                )},
...
def expansion_automatica(G, diccionario, expansion_activa, modo_expansion, usar_wikipedia, usar_gpt):
    ...
    while expansion_activa:
        ...
        if modo_expansion == "p": # and usar_gpt == "s":
            ...
            for nodo in nodos_a_expandir:
                ...
                añadir_a_visualizacion(nodo, [], G)  # sin expansión aún
            print(f"🔬 IA_m está enfocado en: {', '.join(nodos_a_expandir)}")
            if IA_m_Cargado == False:
                visualizar_red(G)
                IA_m_Cargado = True
            guardar_visualizacion_dinamica()  # se genera el HTML solo con IA_m y los 5 nodos
    ...
    try:
        while True:
            ...
            if entrada == "salir":
                ...
                modo_expansion = input("¿Deseas usar expansión basada en prioridad (p) o modelo de atención IA_m (i)?: ").strip().lower()
